...
import { GroupWithMembers, GroupUserSearchResult } from '../../types/groups';

interface MyGroupMembersTabProps {
  groupId: string;
  canEdit: boolean;
}

const GROUP_ROLES: Array<{ value: 'admin' | 'member' | 'viewer'; labelKey: string }> = [
  { value: 'admin', labelKey: 'groups.members.roleAdmin' },
  { value: 'member', labelKey: 'groups.members.roleMember' },
  { value: 'viewer', labelKey: 'groups.members.roleViewer' },
];

const MyGroupMembersTab: React.FC<MyGroupMembersTabProps> = ({ groupId, canEdit }) => {
  ...
  // Fetch group details with members (user-level endpoint)
  const {
    data: groupDetails,
    isLoading,
    error,
  } = useQuery<GroupWithMembers>(
    ['my-group-details', groupId],
  ...
  // Search users for adding (user-level endpoint scoped to group)
  const { data: userSearchResults } = useQuery<GroupUserSearchResult>(
    ['group-user-search', groupId, userSearchValue],
    () => groupsService.searchGroupUsers(groupId, userSearchValue),
    {
  ...
  // Add member mutation (user-level endpoint)
  const addMemberMutation = useMutation({
    mutationFn: ({ userId, role }: { userId: string; role: 'admin' | 'member' | 'viewer' }) =>
      groupsService.inviteMember(groupId, { userId, role }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['my-group-details', groupId] });
  ...
  // Update role mutation (user-level endpoint)
  const updateRoleMutation = useMutation({
    mutationFn: ({ userId, role }: { userId: string; role: 'admin' | 'member' | 'viewer' }) =>
      groupsService.updateGroupMemberRole(groupId, userId, { role }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['my-group-details', groupId] });
      addNotification({
  ...
  // Remove member mutation (user-level endpoint)
  const removeMemberMutation = useMutation({
    mutationFn: (userId: string) => groupsService.removeGroupMember(groupId, userId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['my-group-details', groupId] });
      queryClient.invalidateQueries({ queryKey: ['my-groups'] });
  ...
  const searchResults = userSearchResults?.users || [];
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          >
            ...
            <FlexItem style={{ minWidth: '250px', flex: 1 }}>
              <Select
                role="listbox"
                id="my-group-user-search-select"
            ...
            <FlexItem>
              <Select
                role="listbox"
                id="my-group-new-member-role-select"
...
export default MyGroupMembersTab;
