...
const UserBudgetLimitsTab: React.FC<UserBudgetLimitsTabProps> = ({ userId, canEdit }) => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: UserBudgetLimitsUpdate) =>
      usersService.updateUserBudgetLimits(userId, data),
    onSuccess: () => {
    ...
    <Form style={{ paddingTop: '1rem' }}>
      ...
      >
        <Progress
          ...
          variant={budgetUtilization > 80 ? (budgetUtilization > 95 ? 'danger' : 'warning') : undefined}
        />
        <HelperText>
          <HelperTextItem>
      ...
      {/* Max Budget Input */}
      <FormGroup
        label={t('users.budget.maxBudget', 'Max Budget')}
        fieldId="max-budget"
      >
        <NumberInput
          id="max-budget"
          ...
          isDisabled={!canEdit || updateMutation.isPending}
          aria-label={t('users.budget.maxBudget', 'Max Budget')}
          widthChars={10}
        />
        <HelperText>
          <HelperTextItem>
            {t('users.budget.maxBudgetHelp', 'Maximum spending limit in USD. Set to 0 for unlimited.')}
          </HelperTextItem>
        </HelperText>
      </FormGroup>
      ...
      {/* TPM Limit */}
      <FormGroup
        label={t('users.budget.tpmLimit', 'TPM Limit')}
        fieldId="tpm-limit"
      >
        <NumberInput
          id="tpm-limit"
      ...
      {/* RPM Limit */}
      <FormGroup
        label={t('users.budget.rpmLimit', 'RPM Limit')}
        fieldId="rpm-limit"
      >
        <NumberInput
          id="rpm-limit"
      ...
      {canEdit && (
        <div style={{ marginTop: '1rem' }}>
          <Button
            variant="primary"
            onClick={handleSave}
            isDisabled={!hasChanges || updateMutation.isPending}
      ...
      {!canEdit && (
        <Content component={ContentVariants.small} style={{ marginTop: '1rem', fontStyle: 'italic' }}>
