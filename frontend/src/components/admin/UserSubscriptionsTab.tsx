...
const UserSubscriptionsTab: React.FC<UserSubscriptionsTabProps> = ({ userId }) => {
  ...
  if (subscriptions.length === 0) {
    return (
      <EmptyState>
        <EmptyStateHeader
          titleText={t('users.subscriptions.noSubscriptions', 'No Subscriptions')}
          headingLevel="h4"
          ...
          {subscriptions.map((sub) => (
            <Tr key={sub.id}>
              ...
              <Td dataLabel={t('users.subscriptions.status', 'Status')}>
                <Label color={getStatusColor(sub.status)}>
                  {getStatusLabel(sub.status)}
                </Label>
                {sub.statusReason && (
                  <div style={{ marginTop: '0.25rem', fontSize: 'var(--pf-t--global--font--size--sm)', color: 'var(--pf-t--global--text--color--subtle)' }}>
                    {sub.statusReason}
                  </div>
                )}
              </Td>
